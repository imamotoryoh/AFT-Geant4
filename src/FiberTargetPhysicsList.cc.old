...
//#include "FiberTargetMinEkineCuts.hh"


FiberTargetPhysicsList::FiberTargetPhysicsList()
  : G4VUserPhysicsList(), 
...
FiberTargetPhysicsList::FiberTargetPhysicsList(FiberTargetParamManager* pman)
  : G4VUserPhysicsList(),
    defaultCutValue(0.1*cm),
...
void FiberTargetPhysicsList::ConstructParticle()
{
  ConstructBosons();
  ConstructLeptons();
  ConstructMesons();
...
void FiberTargetPhysicsList::ConstructOp()
{
  //int fVerboseLebel = 1;
...
void FiberTargetPhysicsList::SetNbOfPhotonsCerenkov(G4int MaxNumber)
{
  //G4Cerenkov::SetMaxNumPhotonsPerStep(MaxNumber);
}

void FiberTargetPhysicsList::ConstructBosons()
{
...
void FiberTargetPhysicsList::ConstructLeptons()
{
...
void FiberTargetPhysicsList::ConstructMesons()
{
  ...
  //particle2  = new G4ParticleDefinition("12XiBe",       // Name
  particle2  = new G4ParticleDefinition("pesudoXi",       // Name
					//11.57421*GeV, // Mass ("12XiBe")
					2.259982*GeV, // Mass (2*lambda + 0.028616 GeV)
					0.0*MeV,      // Width
					0.0*eplus,    // Charge
					0,            // Spin
					0,            // Parity
  ...
  particle3  = new G4ParticleDefinition("12XiBe",       // Name
					11.57421*GeV, // Mass ("12XiBe" with B=0 MeV)
					0.0*MeV,      // Width
					0.0*eplus,    // Charge
					0,            // Spin
					0,            // Parity
					...
					NULL,         // DecayTable
					false,        // Shortlived
					"hypernucleus",// Sub Type
					0,            // Antiencoding
					0.0);        // MagneticMoment
  
  // ----- Decay setting for pesudoXi -----
  G4DecayTable *decayTable =  new G4DecayTable();;
  G4VDecayChannel *mode;
  mode      = new G4PhaseSpaceDecayChannel("pesudoXi", 1.0, 2, "lambda", "lambda" );
  decayTable->Insert( mode );
  particle2->SetDecayTable( decayTable );
  
  // ----- Decay setting for 12XiBe -----
  G4DecayTable *decayTable_hyp = new G4DecayTable();
  G4VDecayChannel *mode_hyp;
  mode_hyp  = new G4PhaseSpaceDecayChannel("12XiBe", 1.0, 3, "10Be", "lambda", "lambda" );
  //mode_hyp  = new G4PhaseSpaceDecayChannel("12XiBe", 1.0, 3, "11Be", "lambda", "lambda" );
  //mode_hyp  = new G4PhaseSpaceDecayChannel("12XiBe", 1.0, 3, "e+", "e+", "e-" );
  decayTable_hyp->Insert( mode_hyp );
  particle3->SetDecayTable( decayTable_hyp );
...
void FiberTargetPhysicsList::ConstructBaryons()
{
...
/*void FiberTargetPhysicsList::ConstructHeavyIon()
{
  //Ions
  // Deuteron
...
void FiberTargetPhysicsList::ConstructProcess()
{
  ...
  while( (*theParticleIterator)() ){
    G4ParticleDefinition* particle = theParticleIterator->value();
    G4ProcessManager* pmanager = particle->GetProcessManager();
    pmanager->AddDiscreteProcess(new FiberTargetMinEkineCuts());
...
/////Transportation
void FiberTargetPhysicsList::AddTransportation()
{
  FiberTargetTransportation* theTransportationProcess= new FiberTargetTransportation();
  ...
  while( (*theParticleIterator)() ){
    ...
    if ( !particle->IsShortLived() ) {
      ...
      if ( pmanager == 0) {
        ...
        //G4Exception("FiberTargetPhysicsList::AddTransportation : no process manager!");
	G4cout << "FiberTargetPhysicsList::AddTransportation : no process manager!" << G4endl;
      } 
      else {
...
///////Cut
void FiberTargetPhysicsList::SetCuts()
{
...
void FiberTargetPhysicsList::ConstructEM()
{
  auto theParticleIterator=GetParticleIterator(); //added for ver.10.03
  theParticleIterator->reset();
  while( (*theParticleIterator)() ){
...
void FiberTargetPhysicsList:: ConstructHadronic()
{
  //Elastic models
...
void FiberTargetPhysicsList::ConstructDecay()
{
