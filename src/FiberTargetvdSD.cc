...
FiberTargetvdSD::FiberTargetvdSD( G4String name,
		    G4int nCells,
		    G4String colName )
...
FiberTargetvdSD::~FiberTargetvdSD()
{
  delete [] CellID; //interger
}

void FiberTargetvdSD::Initialize(G4HCofThisEvent*)
{
  VDCollection = new FiberTargetvdHitsCollection
...
G4bool FiberTargetvdSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  FiberTargetvdHit* vdHit =
    new FiberTargetvdHit(physVol->GetLogicalVolume());
  vdHit->SetEdep( edep );
...
void FiberTargetvdSD::EndOfEvent(G4HCofThisEvent*HCE)
{
  if(HCID<0)
...
void FiberTargetvdSD::clear()
{
} 

void FiberTargetvdSD::DrawAll()
{
} 

void FiberTargetvdSD::PrintAll()
{
} 
